...
v.1 - Nicu FLORICA (niq_ro) changed to i2c interface for LCD2004
...
v.2.a - replaced a DHT22 sensor with combi AHT20+BMP280: https://docs.cirkitdesigner.com/component/3d8e6da8-841a-47a6-a1aa-0aa902c905e1/aht20bmp280
*/
 ...
 */
   ...
   aht.begin();
 //    extt = round(aht.getTemperatureSensor() - 1)
 //    delay(200);
 //    exth = round(aht.getHumiditySensor());    

   sensors_event_t humidity, temp;
   aht.getEvent(&humidity, &temp);// populate temp and humidity objects with fresh data
   extt = temp.temperature;
   exth = humidity.relative_humidity;

   bmp.begin(0x76);
   extp = (float)(bmp.readPressure() / 100.0F)*0.75006; // Convertfrom hPa to mmHg

   // Check if any reads failed and exit early (to try again).
   if (isnan(exth) || isnan(extt) || isnan(extp)) 
   {
    Serial.println(F("Failed to read from AHT20/BMP sensor !"));
    lcd.setCursor(16, 3);
    lcd.print("FAIL");
    extaht = false;
    extbmp = false;  
        ...
        if (extaht && extbmp){
         ...
         //  exth = round(aht.getHumiditySensor());    
          ...
          exth = humidity.relative_humidity;
           ...
           extp = 12 + (float)(bmp.readPressure() / 100.0F)*0.75006; // Convert from hPa to mmHg
